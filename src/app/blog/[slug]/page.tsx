...
function renderMarkdown(md: string): string {
  return md
    ...
    .replace(/^(?!<[hulo])(.+)$/gm, '<p class="text-stone-700 leading-relaxed my-3">$1</p>')
    .replace(/<p[^>]*>\s*<\/p>/g, '')
...
export default async function BlogPost({ params }: Props) {
  ...
  return (
    <div className="grid gap-6">
      <div className="rounded-3xl border border-black/20 bg-[#fef8ea] p-8">
        ...
        {frontmatter.excerpt && (
          <p className="mt-3 text-lg text-stone-600">{frontmatter.excerpt}</p>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-black/10 bg-[#0f121e] p-6 text-amber-100">
        <p className="font-semibold">Want Gary to review your project?</p>
        <p className="mt-1 text-sm text-amber-100/70">
          €20 · Personalized audit · Delivered to your inbox within 24h
        </p>
        <Link
          href="/checkout"
